...
void im2col_gpu(const Dtype *data_im,
    ...
    viennacl::ocl::program &prog = (Caffe::Get().GetDevice(dev_id, false))->program();
    ...
    uint mem_base_address_align;
    clGetDeviceInfo(ctx.devices()[dev_id].id(), CL_DEVICE_MEM_BASE_ADDR_ALIGN, sizeof(uint), &mem_base_address_align, NULL);
    ...
    viennacl::ocl::enqueue(
        kernel(num_kernels, WrapHandle(bufDataIm, &ctx), offset_offset_data_im, height, width,
               kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
               dilation_w, height_col, width_col, WrapHandle(bufDataCol, &ctx),
               offset_offset_data_col),
               ctx.get_queue());
}
...
void col2im_gpu(const Dtype *data_col, const int_tp channels,
    ...
    viennacl::ocl::enqueue(
        kernel(num_kernels, WrapHandle(bufDataCol, &ctx), offset_offset_data_col, height,
...
void im2col_nd_gpu(const Dtype* data_im, const int_tp num_spatial_axes,
  ...
  viennacl::ocl::enqueue(
      kernel(num_kernels, num_spatial_axes, (int_tp)0,
             WrapHandle(buf_data_im.memobj, &ctx), (int_tp)buf_data_im.offset, WrapHandle(buf_im_shape.memobj, &ctx),
             WrapHandle(buf_col_shape.memobj, &ctx), WrapHandle(buf_kernel_shape.memobj, &ctx),
             WrapHandle(buf_pad.memobj, &ctx), WrapHandle(buf_stride.memobj, &ctx),
             WrapHandle(buf_dilation.memobj, &ctx), WrapHandle(buf_data_col.memobj, &ctx),
             (int_tp)buf_data_col.offset),
      ctx.get_queue());
}

template void im2col_nd_gpu<float>(const float* data_im, const int_tp num_spatial_axes,
								   const int_tp num_kernels, const int_tp* im_shape,
								   const int_tp* col_shape, const int_tp* kernel_shape,
								   const int_tp* pad, const int_tp* stride,
								   const int_tp* dilation, float* data_col);
								                   
template void im2col_nd_gpu<double>(const double* data_im, const int_tp num_spatial_axes,
								    const int_tp num_kernels, const int_tp* im_shape,
								    const int_tp* col_shape, const int_tp* kernel_shape,
								    const int_tp* pad, const int_tp* stride,
...
void col2im_nd_gpu(const Dtype* data_col, const int_tp num_spatial_axes,
  ...
  viennacl::ocl::enqueue(
      kernel(im_size, num_spatial_axes, (int_tp)0,
             WrapHandle(buf_data_col.memobj, &ctx), (int_tp)buf_data_col.offset, WrapHandle(buf_im_shape.memobj, &ctx),
             WrapHandle(buf_col_shape.memobj, &ctx), WrapHandle(buf_kernel_shape.memobj, &ctx),
             WrapHandle(buf_pad.memobj, &ctx), WrapHandle(buf_stride.memobj, &ctx),
             WrapHandle(buf_dilation.memobj, &ctx), WrapHandle(buf_data_im.memobj, &ctx),
             (int_tp)buf_data_im.offset),
      ctx.get_queue());
}                   	
                                             
template void col2im_nd_gpu<float>(const float* data_col, const int_tp num_spatial_axes,
								   const int_tp im_size, const int_tp* im_shape,
								   const int_tp* col_shape, const int_tp* kernel_shape,
								   const int_tp* pad, const int_tp* stride,
								   const int_tp* dilation, float* data_im);
								                   
template void col2im_nd_gpu<double>(const double* data_col, const int_tp num_spatial_axes,
								    const int_tp im_size, const int_tp* im_shape,
								    const int_tp* col_shape, const int_tp* kernel_shape,
								    const int_tp* pad, const int_tp* stride,
