...
void caffe_gpu_gemm<float>(const CBLAS_TRANSPOSE TransA,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    clEnqueueUnmapMemObject(ctx.get_queue().handle().get(), Mem_A, Aptr, 0, NULL,
    NULL);
    clEnqueueUnmapMemObject(ctx.get_queue().handle().get(), Mem_B, Bptr, 0, NULL,
    NULL);
    clEnqueueUnmapMemObject(ctx.get_queue().handle().get(), Mem_C, Cptr, 0, NULL,
    NULL);
  } else {
    ...
    viennacl::matrix_base<float, size_t, ptrdiff_t> matA(Mem_A, ctx, A_size1,
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(M), A_size2,
                                                       size_type(offA),
    ...
    viennacl::matrix_base<float, size_t, ptrdiff_t> matB(Mem_B, ctx, B_size1,
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(K), B_size2,
                                                       size_type(offB),
    ...
    viennacl::matrix_base<float, size_t, ptrdiff_t> matC(Mem_C, ctx, size_type(M),
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(M),
                                                       size_type(N),
...
void caffe_gpu_gemm<double>(const CBLAS_TRANSPOSE TransA,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    viennacl::matrix_base<double, size_t, ptrdiff_t> matA(Mem_A, ctx, A_size1,
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(M), A_size2,
                                                       size_type(offA),
    ...
    viennacl::matrix_base<double, size_t, ptrdiff_t> matB(Mem_B, ctx, B_size1,
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(K), B_size2,
                                                       size_type(offB),
    ...
    viennacl::matrix_base<double, size_t, ptrdiff_t> matC(Mem_C, ctx, size_type(M),
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(M),
                                                       size_type(N),
...
void caffe_gpu_gemv<float>(const CBLAS_TRANSPOSE TransA, const int_tp M,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    viennacl::vector_base<float, size_t, ptrdiff_t> v2(
        ...
        difference_type(1), ctx);
    viennacl::matrix_base<float, size_t, ptrdiff_t> mat(Mem_A, ctx, size_type(M),
                                                      size_type(0),
                                                      difference_type(1),
                                                      size_type(M),
                                                      size_type(N),
...
void caffe_gpu_gemv<double>(const CBLAS_TRANSPOSE TransA, const int_tp M,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    double* xptr = reinterpret_cast<double*>(clEnqueueMapBuffer(
        ctx.get_queue().handle().get(), Mem_x, true, CL_MAP_READ,
        sizeof(double) * offx, sizeof(double) * (TransA == CblasTrans) ? M : N, 0,
        NULL,
        NULL, NULL));
    double* yptr = reinterpret_cast<double*>(clEnqueueMapBuffer(
        ctx.get_queue().handle().get(), Mem_y, true, CL_MAP_READ | CL_MAP_WRITE,
        sizeof(double) * offy, sizeof(double) * (TransA == CblasTrans) ? N : M, 0,
        NULL,
        NULL, NULL));
    ...
    viennacl::vector_base<double, size_t, ptrdiff_t> v2(
        Mem_y, size_type((TransA == CblasTrans) ? N : M), size_type(offy),
        difference_type(1), ctx);
    viennacl::matrix_base<double, size_t, ptrdiff_t> mat(Mem_A, ctx, size_type(M),
                                                      size_type(0),
                                                      difference_type(1),
                                                      size_type(M),
                                                      size_type(N),
...
void caffe_gpu_memcpy(const uint_tp N, const void* X, void* Y) {
  ...
  else dev_id = clState.get_mem_dev(bufY.memobj);
  
  viennacl::ocl::context &ctx = viennacl::ocl::get_context(dev_id);

  if (bufX.memobj != NULL && bufY.memobj != NULL) {
    clEnqueueCopyBuffer(ctx.get_queue().handle().get(), bufX.memobj, bufY.memobj, bufX.offset,
        bufY.offset, N, 0, NULL, NULL);
  } else if (bufX.memobj != NULL) {
    clEnqueueReadBuffer(ctx.get_queue().handle().get(), bufX.memobj, CL_TRUE, bufX.offset, N,
        Y, 0, NULL, NULL);
  } else if (bufY.memobj != NULL) {
    clEnqueueWriteBuffer(ctx.get_queue().handle().get(), bufY.memobj, CL_TRUE, bufY.offset, N,
        X, 0, NULL, NULL);
  } else {
    memcpy(Y, X, N);
  }
...
void caffe_gpu_sign(const int_tp n, const Dtype* x, Dtype* y) {
  ...
  viennacl::ocl::enqueue(
      oclk_sign(n, WrapHandle(Mem_x, &ctx), offx, WrapHandle(Mem_y, &ctx), offy),
      ctx.get_queue());
}
          
template void caffe_gpu_sign<float>(const int_tp n, const float* x, float* y);
template void caffe_gpu_sign<double>(const int_tp n, const double* x, double* y);
...
void caffe_gpu_sgnbit(const int_tp n, const Dtype* x, Dtype* y) {
  ...
  viennacl::ocl::enqueue(
      oclk_sgnbit(n, WrapHandle(Mem_x, &ctx), offx, WrapHandle(Mem_y, &ctx), offy),
      ctx.get_queue());
}

template void caffe_gpu_sgnbit<float>(const int_tp n, const float* x, float* y);
template void caffe_gpu_sgnbit<double>(const int_tp n, const double* x, double* y);
...
void caffe_gpu_add_scalar(const int_tp N, const float alpha, float* Y) {
  ...
  viennacl::ocl::kernel &oclk_add_scalar = program.get_kernel(
      ...
      "add_scalar" "_float");
  viennacl::ocl::enqueue(oclk_add_scalar(N, alpha, WrapHandle(Mem_Y, &ctx), offY),
                         ctx.get_queue());
}                                    
...
void caffe_gpu_powx<float>(const int_tp N, const float* a, const float alpha,
  ...
  viennacl::ocl::kernel &oclk_powx = program.get_kernel(
      "powx" "_float");
  viennacl::ocl::enqueue(
      oclk_powx(N, WrapHandle(Mem_a, &ctx), offa, alpha, WrapHandle(Mem_y, &ctx), offy),
      ctx.get_queue());
}                      
