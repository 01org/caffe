...
struct ClState::Impl {  
    ...
    void* create_buffer(int dev_id, cl_mem_flags flags, size_t size, void* host_ptr, cl_int &errcode) {
        ...
        cl_mem memobj = clCreateBuffer(ctx.handle().get(), flags, size, host_ptr, &errcode);
...
void* ClState::create_buffer(int dev_id, cl_mem_flags flags, size_t size, void* host_ptr, cl_int &errcode) {
