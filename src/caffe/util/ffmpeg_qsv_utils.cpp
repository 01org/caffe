...
void QsvVideoReader::getVideoFrame(cv::Mat &cv_img) {
    decodeFrame();
    if (frame.data) {
        IplImage imgFrame;
...
void QsvVideoReader::setResize(int width, int height) {
    out_width = width;
    out_height = height;
}
    ...
    while (!valid)
    {
        ...
        if(got_picture)
        {
            ...
            if (!filter_ctx.initiallized) {
                ret = init_filter(&filter_ctx, filter_ctx.dec_ctx, av_asprintf("vpp_qsv=w=%d:h=%d", out_width == 0 ? filter_ctx.dec_ctx->width : out_width, out_height == 0 ? filter_ctx.dec_ctx->height : out_height));
                if (ret < 0) {
